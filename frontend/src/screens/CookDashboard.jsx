...
function OrderCard({ order, onStatusChange, token }) {
  ...
  return (
    <div className="yum" style={{
      ...
      {order.status === 'ready' && <div style={{ fontSize: 13, color: 'var(--yum-online)', fontWeight: 600 }}>Handed to rider — done!</div>}
...
function DishManagerTab({ cook, token }) {
  ...
  function startAdd() {
    setForm({ name: '', subtitle: '', price: '', veg: true, tag: '' });
    setEditingId(null); setShowAdd(true);
  }

  function startEdit(dish) {
    setForm({ name: dish.name, subtitle: dish.subtitle || '', price: String(dish.price), veg: !!dish.veg, tag: dish.tag || '' });
  ...
  async function saveDish() {
    ...
    setSaving(true);
    try {
      const body = { name: form.name.trim(), subtitle: form.subtitle.trim() || null, price: Number(form.price), veg: form.veg, tag: form.tag || null };
      if (editingId) {
  ...
  return (
    <div className="yum" style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--yum-cream)' }}>
      <div style={{ width: '100%', maxWidth: 420, padding: '0 28px' }}>
        ...
        <YButton variant="primary" size="lg" full onClick={() => setShowAuthModal(true)}>
          Sign in with phone
        </YButton>
